  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-tl from-sky-500 to-fuchsia-500">
      ...
      <form className="px-16 py-10 border-2 rounded-lg bg-white shadow-lg flex flex-col" onSubmit={handleSubmit(submitHandler)}>
        <h3 className="text-center text-7xl font-extrabold mb-4" style={{ fontFamily: "Josefin Sans" }}>{isSignup ? "Sign up" : "Sign in"}</h3>
        {/* <div> */}
        {isSignup && (
          <>
        ...
        {/* </div> */}
        <div className="mb-4">
          <button className="rounded-full px-8 py-2 bg-blue-700 text-white font-bold float-right" type="submit">{isSignup ? "Sign Up" : "Sign In"}</button>
        </div>
      </form>
      <button className="text-center mt-3 font-medium" onClick={switchMode}>{isSignup ? "Already have an account? Sign in" : "Don't have an account? Sign Up"}</button>
      {/* </div> */}
    </div>
  );
};
